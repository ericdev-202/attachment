...
def viewallstudent(request):
    user = request.user
    dict={
    'total_student':StudentDetails.objects.filter(university_name=user.university_name).count(),
    'total_student_company':CompDetails.objects.filter(university_name=user.university_name).count(),
    'total_student_assessed':Student.objects.filter(status="assessed",university_name=user.university_name).count(),
    'total_student_assessed_lecturer':Lecturer.objects.filter(status="assessed",university_name=user.university_name).count(),
...
def lecturerassement(request):
    ...
    return render(request,'lectures/lecturerassess.html',{'students':students})
...
@login_required
def companystudent(request):
    user = request.user
    dict={
    'total_students_company':CompDetails.objects.filter(company_name=user.company_name).count(),
    'total_students_assessed':Student.objects.filter(status="assessed",company_name=user.company_name).count(),
    'total_students_notassessed':Student.objects.filter(status="notassessed",company_name=user.company_name).count(),
    'total_students_assessed_lecturer':Lecturer.objects.filter(status="assessed",company_name=user.company_name).count(),
    'students':CompDetails.objects.filter(company_name=user.company_name),
    }
    
    # if request.method == 'GET':
    #     students = StudentDetails.objects.all()
    return render(request,'supervisors/companystudents.html',context=dict)
...
@login_required
def viewassess(request, id):
    viewlecasssess = Student.objects.get(id=id)
    return render(request,'students/viewassessment.html',{'viewlecasssess':viewlecasssess})
...
def editcompany(request, id):
    companydetails = CompDetails.objects.get(id=id)
    return render(request,'students/editcompanydetails.html',{'companydetails':companydetails})

def updatecompany(request,id):    
    companydetails = CompDetails.objects.get(id=id)
    form = CompDetailsForm(request.POST, instance=companydetails)
    if form.is_valid():
        form.save()
...
# @login_required
# def editcompany(request, id):  
#     student = CompDetails.objects.get(id=id)  
#     supervisor = Student.objects.get(id=id)
#     lecturer = Lecturer.objects.get(id=id)
#     return render(request,'students/editcompany.html', {'student':student,'supervisor':supervisor,'lecturer':lecturer})  
...
#     supervisor = Student.objects.get(id=id)
#     lecturer = Lecturer.objects.get(id=id)
#     form = CompDetailsForm(request.POST, instance = student)  
#     s_form = CompanyF1Form(request.POST,instance = supervisor)
#     l_form = LecturerF1Form(request.POST,instance = lecturer)
#     if form.is_valid() and s_form.is_valid() and l_form.is_valid():  
#         form.save() 
#         s_form.save() 
#         l_form.save()
#         return redirect('ViewCompany')  
...
def deletecompany(request):
    id = request.GET.get('id',None)
    CompDetails.objects.get(id=id).delete()
